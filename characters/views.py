...
def get_random_character_view(request: Request) -> Response:
    ...
    pks = Character.objects.values_list(
        "pk", flat=True
    )  # Возврат в виде списка [1, 2, ...]
...
class CharacterListView(generics.ListAPIView):
    # queryset = Character.objects.all()  # При изменении get_queryset не нужен
    ...
    @extend_schema(
        parameters=[
            OpenApiParameter(
                name="name",
                description="Filter by name intransitive contains",
                required=False,
                type=str,
            ),
        ]
